...
class AppointmentsResource extends BaseResource {
    ...
    async get(id, { id, state_at }) {
        const { id, ...queryParams } = this.schemas.get.parse({ id, state_at })
        const params = { id };
        return this.apiClient.get(this.endpoints.GET, { params, queryParams });
    }

    async update(id, { at, to, edit_at, employee, services, client_with_creation, client, recurrence_interval, notes, client_notes, confirmed, status, custom_fields, update_recurrence_flag, send_client_confirmation_email, send_employee_notification_email, user_type, api_options, reassign_employee_if_necessary }) {
        const { id, ...body } = this.schemas.create.parse({ id, at, to, edit_at, employee, services, client_with_creation, client, recurrence_interval, notes, client_notes, confirmed, status, custom_fields, update_recurrence_flag, send_client_confirmation_email, send_employee_notification_email, user_type, api_options, reassign_employee_if_necessary })
        const params = { id };
        return this.apiClient.patch(this.endpoints.UPDATE, { params, body });
    }

    async remove(id, { at, delete_future_occurences, send_client_cancel_email, send_employee_notification_email, user_type, cancellation_message }) {
        const { id, ...queryParams } = this.schemas.remove.parse({ id, at, delete_future_occurences, send_client_cancel_email, send_employee_notification_email, user_type, cancellation_message })
        const params = { id };
        return this.apiClient.delete(this.endpoints.REMOVE, { params, queryParams });
    }

    async accept(id, { notify_client, accept_with_conflict }) {
        const { id, ...body } = this.schemas.accept.parse({ id, notify_client, accept_with_conflict })
        const params = { id };
        return this.apiClient.patch(this.endpoints.ACCEPT, { params, body });
    }

    async decline(id, { notify_client }) {
        const { id, ...body } = this.schemas.decline.parse({ id, notify_client })
        const params = { id };
